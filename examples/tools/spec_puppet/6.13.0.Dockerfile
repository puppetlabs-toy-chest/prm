...
### docker build -t pdk:puppet-6.13.0-puppet_spec -f examples/tools/spec_puppet/6.13.0.Dockerfile .

# cache gems:
### docker run --rm -v ${PWD}:/code -v C:\Users\dave\.pdk\prm\cache:/cache pdk:puppet-6.13.0-spec_cache

# step 1:
### docker run --rm -v ${PWD}:/code -v C:\Users\dave\.pdk\prm\cache:/cache:ro -w /code pdk:puppet-6.13.0-puppet_spec spec_prep

# step 2
### docker run --network none --rm -v ${PWD}:/code -v C:\Users\dave\.pdk\prm\cache:/cache:ro -w /code pdk:puppet-6.13.0-puppet_spec
